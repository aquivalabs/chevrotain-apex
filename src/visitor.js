...
class SQLToAstVisitor extends BaseSQLVisitor {
  ...
  compilationUnit(ctx) {
    let annotations = []
    if (ctx.annotation) {
      ctx.annotation.map((annotation) => annotations.push(this.visit(annotation)))
    }
    ...
    if (ctx.importDeclaration) {
      ctx.importDeclaration.map((importDeclaration) => imports.push(this.visit(importDeclaration)))
    }
    const types = []
    if (ctx.typeDeclaration) {
      ctx.typeDeclaration.map((typeDeclaration) => types.push(this.visit(typeDeclaration)))
  ...
  typeDeclaration(ctx) {
    ...
    if (ctx.classOrInterfaceModifier) {
      ctx.classOrInterfaceModifier.map((modifier) => modifiers.push(this.visit(modifier)))
    }
  ...
  annotation(ctx) {
    ...
    if (hasBraces) {
      if (ctx.expression) {
        ...
        values = []
        const elementValueArrayInitializer = this.visit(ctx.elementValueArrayInitializer)
        values.push(elementValueArrayInitializer)
      }

      if (ctx.elementValuePair) {
        ctx.elementValuePair.map((elementValuePair) => values.push(this.visit(elementValuePair)))
      }
    }
  ...
  elementValuePairs(ctx) {
    const pairs = []
    if (ctx.elementValuePair) {
      ctx.elementValuePair.map((elementValuePair) => pairs.push(this.visit(elementValuePair)))
    }
  ...
  elementValueArrayInitializer(ctx) {
    const elementValues = []
    if (ctx.elementValue) {
      ctx.elementValue.map((elementValue) => elementValues.push(this.visit(elementValue)))
    }
    return {
  ...
  typeParameters(ctx) {
    ...
    if (ctx.typeParameter) {
      ctx.typeParameter.map((typeParameter) => parameters.push(this.visit(typeParameter)))
    }
  ...
  memberDeclaration(ctx) {
    ...
    if (ctx.genericMethodDeclarationOrGenericConstructorDeclaration) {
      return this.visit(ctx.genericMethodDeclarationOrGenericConstructorDeclaration)
    }
    if (ctx.fieldDeclarationOrMethodDeclarationOrConstructorDeclaration) {
      return this.visit(ctx.fieldDeclarationOrMethodDeclarationOrConstructorDeclaration)
    }
  }
  ...
  fieldDeclarationOrMethodDeclarationOrConstructorDeclaration(ctx) {
    if (ctx.Identifier) {
      ...
      if (
        ...
        // methodDeclaration
        const name = this.identifier(ctx.Identifier[ctx.Identifier[0].isMethodDeclaration ? 1 : 0])
        const parameters = this.visit(ctx.formalParameters)
        const dimensions = []
        if (ctx.LSquare) {
          ctx.LSquare.map((lSquare) => {
  ...
  constantDeclarationOrInterfaceMethodDeclaration(ctx) {
    ...
    if (ctx.methodBody) {
      ...
      if (ctx.interfaceMethodModifier) {
        ctx.interfaceMethodModifier.map((modifier) => modifiers.push(this.visit(modifier)))
      }
      const typeParameters = this.visit(ctx.typeParameters)
      let typeType = undefined
      if (ctx.typeType) {
  ...
  variableDeclarators(ctx) {
    const list = []
    if (ctx.variableDeclarator) {
      ctx.variableDeclarator.map((variableDeclarator) => list.push(this.visit(variableDeclarator)))
    }
  ...
  annotationTypeBody(ctx) {
    ...
    if (ctx.annotationTypeElementDeclaration) {
      ctx.annotationTypeElementDeclaration.map((annotationTypeElementDeclaration) =>
  ...
  qualifiedNameList(ctx) {
    ...
    if (ctx.qualifiedName) {
      ctx.qualifiedName.map((qualifiedName) => list.push(this.visit(qualifiedName)))
    }
  ...
  identifierList(ctx) {
    const identifiers = []
    if (ctx.Identifier) {
      ctx.Identifier.map((identifierToken) => identifiers.push(identifierToken.image))
    }
  ...
  formalParameterList(ctx) {
    ...
    for (let i = 0; i < formalParameters.length; i++) {
      if (formalParameters[i].dotDotDot && i + 1 < formalParameters.length) {
        throw new MismatchedTokenException('Only last parameter is allowed with "..."', undefined)
      }
    }
  ...
  formalParameter(ctx) {
    ...
    if (ctx.variableModifier) {
      ctx.variableModifier.map((modifier) => modifiers.push(this.visit(modifier)))
    }
    const typeType = this.visit(ctx.typeType)
    const id = this.visit(ctx.variableDeclaratorId)
  ...
  block(ctx) {
    ...
    if (ctx.blockStatement) {
      ctx.blockStatement.map((blockStatement) => blockStatements.push(this.visit(blockStatement)))
    }
  ...
  blockStatement(ctx) {
    ...
    if (ctx.expression) {
      ...
      if (expression.type === 'PRIMITIVE_TYPE') {
        ...
        if (ctx.Colon) {
          throw new MismatchedTokenException('Primitive type with colon found', undefined)
        }
      ...
      if (expression.type !== 'IDENTIFIER') {
        // if expression is only a primitiveType nothing else is allowed with it
        if (ctx.Colon) {
          throw new MismatchedTokenException('Only identifier is allowed with colon', undefined)
        }
        if (ctx.typeArguments || ctx.Dot) {
          throw new MismatchedTokenException(
      ...
      const followedEmptyLine = this.visit(ctx.semiColon).followedEmptyLine
      if (expression.type === 'IDENTIFIER' || expression.type === 'PRIMITIVE_TYPE') {
  ...
  tryStatement(ctx) {
    ...
    const catchClauses = []
    if (ctx.catchClause) {
      ctx.catchClause.map((catchClause) => catchClauses.push(this.visit(catchClause)))
    }
  ...
  catchType(ctx) {
    const types = []
    if (ctx.qualifiedName) {
      ctx.qualifiedName.map((qualifiedName) => types.push(this.visit(qualifiedName)))
    }
  ...
  switchBlockStatementGroup(ctx) {
    ...
    const statements = []
    if (ctx.blockStatement) {
      ctx.blockStatement.map((blockStatement) => statements.push(this.visit(blockStatement)))
    }
  ...
  forControl(ctx) {
    if (ctx.Colon) {
      ...
      if (ctx.variableDeclarator) {
        ctx.variableDeclarator.map((declarator) => declarators.list.push(this.visit(declarator)))
      }
    ...
    if (ctx.semiColon && ctx.semiColon.length == 2) {
      ...
      if (ctx.expression && ctx.expression[ctx.expression.length - 1].optionalExpression) {
        basicForStatement.expression = this.visit(ctx.expression[ctx.expression.length - 1])
      }
  ...
  expression(ctx) {
    if (ctx.atomic) {
      ...
      if (ctx.ifElseExpressionRest && !ctx.operatorExpressionRest && !ctx.qualifiedExpressionRest) {
      ...
      if (ctx.qualifiedExpressionRest) {
        ...
        if (ctx.instanceofExpressionRest) {
          const instanceofExpressionRest = this.visit(ctx.instanceofExpressionRest)
  ...
  qualifiedExpressionRest(ctx) {
    ...
    if (ctx.Identifier) {
      ...
      if (ctx.typeArgumentsOrOperatorExpressionRest) {
        ...
        if (typeArgumentsOrOperatorExpressionRest.type === 'OPERATOR_EXPRESSION_REST') {
          expression = {
            type: 'OPERATOR_EXPRESSION',
            left: expression,
      ...
      if (ctx.expression) {
        if (ctx.variableDeclaratorId) {
          ...
          for (let i = 0; i < ctx.expression.length; i++) {
            ...
            const variableDeclaratorId = this.visit(ctx.variableDeclaratorId[i])

            const modifiers = []
            if (ctx.Final && ctx.Final.find((final) => final.cnt === i) !== undefined) {
              modifiers.push({
                type: 'MODIFIER',
                value: 'final',
              })
  ...
  primary(ctx) {
    ...
    if (ctx.primitiveType) {
      ...
      value = this.visit(ctx.identifierOrIdentifierWithTypeArgumentsOrOperatorExpression)
    }
  ...
  commentStandalone(ctx) {
    ...
    if (ctx.JavaDocCommentStandalone) {
      return this.JavaDocTraditionalCommentStandalone(ctx.JavaDocCommentStandalone[0])
    }

    if (ctx.TraditionalCommentStandalone) {
      return this.JavaDocTraditionalCommentStandalone(ctx.TraditionalCommentStandalone[0])
    }
  }
  ...
  addComment(node, astResult) {
    const hasComment =
      node.children.LineComment || node.children.TraditionalComment || node.children.JavaDocComment
