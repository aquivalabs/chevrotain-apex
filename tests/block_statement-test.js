...
describe('blockStatement', () => {
  it('localVariableDeclaration: primitive', () => {
    expect(Parser.parse('boolean A;', (parser) => parser.blockStatement())).toEqual({
  ...
  it('localVariableDeclaration: one modifier', () => {
    expect(Parser.parse('@Bean boolean A;', (parser) => parser.blockStatement())).toEqual({
      type: 'EXPRESSION_STATEMENT',
      expression: {
        type: 'LOCAL_VARIABLE_DECLARATION',
        modifiers: [
  ...
  it("localVariableDeclaration: wrong modifier 'public'", () => {
    expect(() => Parser.parse('public boolean A;', (parser) => parser.blockStatement())).toThrow(
      MismatchedTokenException
    )
  })

  it("localVariableDeclaration: wrong modifier 'protected'", () => {
    expect(() => Parser.parse('protected boolean A;', (parser) => parser.blockStatement())).toThrow(
      MismatchedTokenException
    )
  })

  it("localVariableDeclaration: wrong modifier 'private'", () => {
    expect(() => Parser.parse('private boolean A;', (parser) => parser.blockStatement())).toThrow(
      MismatchedTokenException
    )
  })

  it("localVariableDeclaration: wrong modifier 'static'", () => {
    expect(() => Parser.parse('static boolean A;', (parser) => parser.blockStatement())).toThrow(
      MismatchedTokenException
    )
  })

  it("localVariableDeclaration: wrong modifier 'abstract'", () => {
    expect(() => Parser.parse('abstract boolean A;', (parser) => parser.blockStatement())).toThrow(
      MismatchedTokenException
    )
  })

  it("localVariableDeclaration: wrong modifier 'strictfp'", () => {
    expect(() => Parser.parse('strictfp boolean A;', (parser) => parser.blockStatement())).toThrow(
      MismatchedTokenException
    )
  })

  it('classDeclaration', () => {
    expect(Parser.parse('class A{}', (parser) => parser.blockStatement())).toEqual({
  ...
  it('localTypeDeclaration: interface', () => {
    expect(Parser.parse('interface A{}', (parser) => parser.blockStatement())).toEqual({
      type: 'LOCAL_TYPE_DECLARATION',
      modifiers: [],
      declaration: {
  ...
  it('localTypeDeclaration: one modifier', () => {
    expect(Parser.parse('public class A{}', (parser) => parser.blockStatement())).toEqual({
      type: 'LOCAL_TYPE_DECLARATION',
      modifiers: [
        {
          type: 'MODIFIER',
  ...
  it('localTypeDeclaration: multiple modifiers', () => {
    expect(Parser.parse('public static class A{}', (parser) => parser.blockStatement())).toEqual({
      type: 'LOCAL_TYPE_DECLARATION',
      modifiers: [
        {
          type: 'MODIFIER',
  ...
  it("System.debug('please work')", () => {
    expect(
      Parser.parse("System.debug('please work');", (parser) => parser.blockStatement())
    ).toEqual({
      type: 'EXPRESSION_STATEMENT',
      expression: {
  ...
  it('return ', () => {
    expect(Parser.parse('return this;', (parser) => parser.blockStatement())).toEqual({
  ...
  it('variableDeclaration', () => {
    expect(
      Parser.parse('final List<Filter> filterList = new ArrayList<>();', (parser) =>
